...
describe("admin google", () => {
  ...
  it("returns 503 from start when the client is not configured", async () => {
    ...
    expect((await (await api("/admin/api/google/status")).json() as any).configured).toBe(false);
  });
  ...
  it("shows disconnected when the stored token no longer decrypts", async () => {
    ...
    const { fetch } = testApp(NOW);
    const api = asAdmin(fetch);
    expect((await (await api("/admin/api/google/status")).json() as any).connected).toBe(false);
  });
  ...
  it("syncs on demand, disconnects, and retries failed outbox rows", async () => {
    ...
    expect((await (await api("/admin/api/google/status")).json() as any).lastSyncAt).toBe(Math.floor(NOW.getTime() / 1000));
