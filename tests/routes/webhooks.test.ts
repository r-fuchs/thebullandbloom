...
describe("POST /webhooks/stripe", () => {
  it("marks the order paid on completion and is idempotent", async () => {
    ...
    payments.nextEvent = { type: "checkout.session.completed", sessionId: "cs_w1", paymentIntent: "pi_w1", taxCents: 680, discountCents: 850 };
    ...
    const row = await env.DB.prepare("SELECT status, stripe_payment_intent, hold_expires_at, tax_cents, discount_cents FROM orders WHERE id = 'w1'").first<any>();
    expect(row).toEqual({ status: "paid", stripe_payment_intent: "pi_w1", hold_expires_at: null, tax_cents: 680, discount_cents: 850 });
  });
  it("resurrects a cancelled (expired-hold) order when completion arrives late, and is idempotent (D17)", async () => {
    ...
    const { fetch, payments } = testApp();
    payments.nextEvent = { type: "checkout.session.completed", sessionId: "cs_w1c", paymentIntent: "pi_w1c", taxCents: 0, discountCents: 0 };
    expect(await (await hook(fetch)).json()).toEqual({ received: true, applied: "paid" });
