...
Still to verify: Uber sandbox dispatch, Instagram feed, subscription portal, and the Cloudflare rate-limit rules (need the zone, so at DNS cutover). That rule list must include `/api/quote`: it is unauthenticated and calls Uber's metered quote API, so abuse would return `429 customer_limited` and degrade every customer to the fallback fee.
