...
| D22 | The OAuth consent screen is published "In production" without Google verification. Anthony sees Google's "unverified app" interstitial once and clicks through. | "Testing" status | Testing-status refresh tokens expire after 7 days, which would silently stop calendar and email weekly. Verification is a multi-day review that can be filed later if the interstitial proves unacceptable. Task 12 checks this on the real consent flow. **Outcome (2026-09-08):** published "In production" as planned. The console refused to publish ("OAuth configuration is incomplete") until a privacy policy link was set on Branding; Google gates publishing of External apps with sensitive scopes on it. Set to https://thebullandbloom.com/privacy, so a real privacy page (site/privacy.html) is now a cutover requirement. Authorized domains: thebullandbloom.com, thebullandbloom.workers.dev. Home page set to https://thebullandbloom.com. |
...
- Same code run locally first in an iPhone-size browser (workerd, local D1): storefront menu and picker, admin login and grid, order deep link, done/undo keeps the day count, admin close removes a day from the picker, cap 1 shows 1/1 and strikes the day through, past-cutoff today is struck through and unselectable, empty-name validation, payments-down 503 releases the hold, subscription form and order form stay independent, `/privacy` serves. 142 tests green, typecheck clean.
- Retry path, end to end: a connected buy (Sept 22) reached the Orders calendar as expected. Disconnect cleared the calendar-closed days (Sept 23–27) and left the admin-closed days alone. A second buy (Sept 29) while disconnected went `paid` with the status endpoint reading `outbox {pending: 3, failed: 0}`. Anthony reconnected through the unverified-app interstitial; the panel then read "3 message(s) waiting to send"; the 15-minute cron drained them: the Sept 29 event appeared on the Orders calendar and the "New order" copy landed in thebullandbloom@gmail.com. The calendar link opened admin on Sept 29 and Anthony marked the order done from his phone.
- Closed-calendar sync from Anthony's phone: "Check the Closed calendar now" restored Sept 23–27; an all-day event for Sept 30 closed it and removed it from the picker; deleting it reopened the day.
- Storefront on real phones: Stripe back arrow returns to the order section; the Formspree inquiry arrived in the store inbox; no courier button or delivery option on the preview (the Sept 9 courier work on Ryan's Mac is not pushed).
- Two admin gaps found and fixed on the branch: the Google panel hid the queued-message count while disconnected, and after a reconnect the only way to send waiting messages was the next cron (the Retry button showed only for failed rows). The panel now shows the count in every state and offers "Send waiting messages now"; the OAuth callback drains the outbox itself (test added; 143 green).
- From Anthony, for §7: keep the three sizes; subscriptions should offer weekly (four a month) and twice a month; prices still to be decided.

Plan 3 (Uber Direct delivery), verified 2026-09-14 on the preview after the merge into `claude/bull-bloom-testing-5heggz` (299 tests green; Robocourier on via the deploy workflow's `uber_robocourier` input; Anthony's Uber Direct organisation, test credentials):

- Quote at checkout: a downtown Albany address priced at $7.99 within a second and the total line read "Bouquet $85 + delivery $7.99 = $92.99"; Seattle and Hudson NY both answered "outside our delivery area"; a date ten weeks out priced as an estimate. Continue stayed disabled until a quote was held.
- Buy it: Stripe Checkout carried two line items (bouquet, delivery); admin showed the order as `paid · delivery` with the address and drop-off note; the confirmation email read "Delivering to: 79 N Pearl St, Albany, NY 12207".
- Request courier: the line became "Courier: pending · $7.99 track" and the tracking email with a working direct-order.uber.com link arrived within a minute. Uber's own tracking page showed the same job.
...
Plan 1 shipped to a preview URL on 2026-09-08 (branch `feat/store`, not merged; merge is the cutover because the page moved into `site/`). What Anthony would notice: a menu with three sizes and a day picker on his site, Stripe taking payment, and an admin page where closing a day takes it off the market instantly. Prices, cap, cutoff, and studio address are SAMPLE values until he supplies his (§7). Plan 2 reached the preview on 2026-09-09. What Anthony would notice now: every paid order appears as an all-day item on his phone's "Bull and Bloom: Orders" calendar with the customer's name and a link into admin; the customer gets a confirmation from his store Gmail and he gets a "New order" copy; an out-of-office on his "Bull and Bloom: Closed" calendar takes those days off the market within fifteen minutes, and admin's own closures always win. Before cutover: Anthony's real prices, cap, pickup text and address (§7), the live Stripe key, trimming Stripe's payment methods, rate-limit rules, and a `/privacy` page that resolves on thebullandbloom.com (the consent screen links to it). The road test of 2026-09-10 closed the last Plan 2 check (retry path) and fixed two admin panel gaps.

Later on 2026-09-10 (branch `claude/bull-bloom-testing-5heggz`, on the preview via the GitHub Actions deploy workflow, 181 tests green): Plan 4 subscriptions shipped end to end for pickup (`docs/superpowers/plans/2026-09-10-store-plan-4-subscriptions.md`; a live weekly signup by Ryan produced the confirmation with a working Stripe portal link); the Instagram carousel (§2 item 7) is built behind an adapter with a recorded-feed test, waiting on a Meta app and on R2 being enabled for the account (the deploy strips the bucket binding until then); the storefront got a design pass (menu cards, calendar day picker, photo strip when a feed exists). Anthony's answers: keep three sizes; weekly and twice-monthly cadences; prices per cell approved (one-time prices still SAMPLE); no by-hand week skips. Stripe's customer portal has no pause, so customers cancel or change cards; Anthony pauses from the Stripe dashboard. The Uber Direct courier work (Plan 3) ran on the preview from Ryan's Mac on 2026-09-09 but was never pushed; it must be committed from the Mac and merged. Plan 3 was merged into this branch on 2026-09-14 and its acceptance walk-through ran the same day on the preview (§8). What Anthony would notice: a Pickup or Delivery choice on the one-time tab with a live price, a Request courier button on each delivery order that books an Uber and emails the customer a tracking link, and the order closing itself when the courier drops off. Before real couriers: his production Uber organisation's keys on the worker (GitHub secrets `UBER_PROD_*` are in place; the workflow step that uploads them at cutover is still to write), and real pickup instructions. Hand-over was decided 2026-09-14: the courier leaves the bouquet at the door, nobody needs to be home. That head (`317e76d`: the at-the-door hand-over, Anthony’s real pickup instructions, the cutover key-swap step in the deploy workflow, `scripts/launch-reset.sql`) reached the preview on 2026-09-14 via Deploy run 22, dispatched from GitHub on `claude/staging-review-bull-bloom-f8x3sw` with the preview URL and Robocourier on: 299 tests green, no migrations to apply, MEDIA bucket bound, `/api/health` OK. What remains before cutover is in the batch-two checklist and §7.

2026-09-15 (Plan 5, `docs/superpowers/specs/2026-09-15-store-plan-5-vase-tax-zones-design.md`): the
first live days surfaced four things and all shipped the same day — a hand-tied or in-a-vase choice
priced per size (+$15/+$20/+$25), Stripe Tax on every session with the delivery address as the tax
location plus promotion codes, delivery zones (Capital District $10, Saratoga $35, Hudson $35) as the
fallback behind Uber pricing, and the Uber token cache bound to its client id after production spent
a day sending a sandbox token (403 `customer_blocked`). Earlier that day: a State placeholder that
customers read as filled in was the "Continue to payment does nothing" report, fixed by prefilling
NY. Uber's account radius is 10 miles from the studio; beyond it the zone fee applies and Anthony
drives. Details and the open items in Plan 5 §6.
